...
result(30) = struct('input_f', [], 'output_d', [], 'output_e', [], 'corr', [], 'R2', [], 'RMSE', []);
parfor random_time=1:30
    ...
    result(random_time).input_f=input_f;
    result(random_time).output_d=output_d;
    result(random_time).output_e=output_e;
    result(random_time).corr=corr(output_e,output_d);
    result(random_time).R2=fitlm(output_e,output_d).Rsquared.Ordinary;
    result(random_time).RMSE=sqrt(mean((output_e - output_d) .^ 2));
    result(random_time).distances_std=distances_std;

    fprintf('random %d is done\n',random_time)
end
toc;